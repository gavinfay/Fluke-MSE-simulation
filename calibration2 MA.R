...
for (p in min_period:max_period) {
  ...
  for(i in 1:10) {
    ...
    sf_catch_data = data.frame(read_excel("MANY_catch_data_sim1.xlsx"))                                                                            
...
pds_new_all_MA= subset(pds_new_all_MA, select=-c(Group.1, tot_sf_catch, tot_bsb_catch, tot_scup_catch))
